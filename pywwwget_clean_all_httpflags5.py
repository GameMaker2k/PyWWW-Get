...
# -*- coding: utf-8 -*-
"""
pywwwget_clean_all_httpflags5.py

Builds on httpflags4 and adds TCP/UDP quality-of-life flags:

TCP receive (download_file_from_internet_file("tcp://...")):
- reuse=1              -> SO_REUSEADDR on listen socket (rebind faster)
- linger=0|1           -> SO_LINGER on listen socket (0 = hard close, 1 = normal linger default)
- accept_timeout=<sec> -> timeout waiting for sender to connect (separate from read timeout)
- port=0               -> OS chooses free port; print_url will show chosen port
- print_url=1          -> prints listening URLs for tcp/udp receive too

UDP receive:
- end_timeout=<sec>    -> stop after this many seconds of inactivity (silence); useful for "done"
- reuse=1              -> SO_REUSEADDR on UDP socket
- port=0 + print_url=1
...
log = logging.getLogger("pywwwget_all_httpflags5")
if not log.handlers:
...
def _parse_opts(url):
    ...
    opts = {
        ...
        "accept_timeout": _gi(q, "accept_timeout", _gi(q, "timeout", 10)),
...
# --------------------------
# HTTP server (same as httpflags4)
# --------------------------
...
def _tcp_recv_to_temp(url):
    ...
    if o["print_url"]:
        query = p.query or ""
        for u in _pretty_urls("tcp", bind, actual_port, (p.path or "/"), ("?"+query if query else "")):
...
def _udp_recv_to_temp(url):
    ...
    if o["print_url"]:
        query = p.query or ""
        for u in _pretty_urls("udp", bind, actual_port, (p.path or "/"), ("?"+query if query else "")):
            sys.stdout.write("Listening: %s\n" % u)
        sys.stdout.flush()
