...
# Optional Bluetooth fallback (PyBluez) for Windows/macOS/older builds
try:
    import bluetooth as _pybluez  # PyBluez
except Exception:
    _pybluez = None
import shutil
import time
...
def _parse_net_url(url):
    ...
    if p.scheme in ("tcp", "bt", "rfcomm", "bluetooth") and not has_timeout:
        timeout = None
    else:
        timeout = float(_qnum(qs, "timeout", 1.0 if p.scheme == "udp" else 30.0, cast=float))
    accept_timeout = float(_qnum(qs, "accept_timeout", 0.0 if p.scheme in ("tcp","bt","rfcomm","bluetooth") else (timeout or 0.0), cast=float))
    ...
    if "connect_wait" in qs:
        ...
        connect_wait = (p.scheme in ("tcp","bt","rfcomm","bluetooth"))
    handshake = _qflag(qs, "handshake", True if p.scheme in ("tcp","udp","bt","rfcomm","bluetooth") else False)
...
# --- Bluetooth RFCOMM helpers (Python 2.7 + 3.x) --------------------------------
...
def _split_bt_netloc(netloc):
    ...
    s = netloc.strip()
    # remove any accidental userinfo "x@y"
    if "@" in s:
        s = s.split("@", 1)[1]
    s = s.replace("-", ":")
    parts = s.split(":")
    # If last token is digits and we have at least 7 tokens, treat last as channel.
    ch = None
...
def _bt_host_channel_from_parts(parts, qs, o):
    '''
    Determine Bluetooth address + RFCOMM channel from parsed URL parts and query.

    Priority:
      1) explicit :channel in netloc (e.g. bt://AA:..:FF:3/..)
      2) query param channel= or rfcomm_channel=
      3) fallback channel 1 if missing/0

    Also supports bind=<local_bdaddr> for listeners.
    '''
    addr, ch = _split_bt_netloc(getattr(parts, "netloc", "") or "")
    # bind override for listeners
...
def recv_to_fileobj(fileobj, host, port, proto="tcp", path_text=None, **kwargs):
    ...
    if proto in ("tcp", "bt", "rfcomm", "bluetooth"):
        if proto in ("bt", "rfcomm", "bluetooth"):
            if not _has_rfcomm():
                try:
                    ...
                    pass
                return False
            # RFCOMM client: connect to remote BDADDR/channel
            sock = _bt_socket_stream()
            if sock is None:
                return False
            try:
                if _norm_bt_addr(host) == "00:00:00:00:00:00":
                    try:
                        sock.close()
                    except Exception:
                        pass
        ...
        while True:
            try:
                if proto in ("bt", "rfcomm", "bluetooth"):
                    sock.connect((_norm_bt_addr(host), int(port)))
                else:
                ...
                if wait_timeout is not None and wait_timeout >= 0 and (time.time() - start_t) >= wait_timeout:
                    ...
                    return False
                try:
                    _net_log(kwargs.get("verbose"), "Stream: waiting for receiver (connect refused), retrying...")
                    time.sleep(0.1)
                except Exception:
                    pass
                continue
...
def download_file_from_internet_file(url, **kwargs):
    ...
    if p.scheme in ("tcp", "udp") or p.scheme in _BT_SCHEMES:
        ...
        if parts.scheme in _BT_SCHEMES:
            ...
            host, port = _bt_host_channel_from_parts(parts, qs, o)
        else:
        ...
        ok = recv_to_fileobj(
            outfile, host=host, port=port, proto=(p.scheme if p.scheme in ("tcp","udp") else "bt"),
...
def upload_file_to_internet_file(fileobj, url):
    ...
    if p.scheme in ("tcp", "udp") or p.scheme in _BT_SCHEMES:
        ...
        if parts.scheme in _BT_SCHEMES:
            qs = parse_qs(parts.query or "")
            # For send/connect, ignore bind= as a remote host override.
            o2 = dict(o)
            try:
                o2["bind"] = None
            except Exception:
                pass
            host, port = _bt_host_channel_from_parts(parts, qs, o2)
        else:
            host = parts.hostname
            port = parts.port or 0
        path_text = parts.path or "/"
        ...
        ok = send_from_fileobj(
            fileobj, host=host, port=port, proto=(p.scheme if p.scheme in ("tcp","udp") else "bt"),
            mode=o.get("mode"), timeout=o.get("timeout"), total_timeout=o.get("total_timeout"),
