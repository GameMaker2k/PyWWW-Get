...
def _udpseq_send_from_fileobj(fileobj, host, port,
    ...
    """
    # keep payload small-ish to avoid fragmentation; user can override chunk_size
    payload_max = int(chunk_size) if int(chunk_size) > 0 else 1024
    ...
    connected = False
    try:
        # connect() lets us use send()/recv() and filters ICMP errors better
        try:
            sock.connect(addr)
            connected = True
        except Exception:
        ...
        seq = 0
        inflight = {}  # seq -> [last_sent_ts, pkt, tries, payload_len]
        eof = False
        sent_bytes = 0
        start_ts = _udpseq_now()
        last_activity = start_ts

        # ensure file at start if possible
        try: fileobj.seek(0, 0)
        except Exception: pass
        ...
        while True:
            ...
            if eof and not inflight:
                ...
                # repeat DONE a few times
                for _ in range(3):
                    _udpseq_send_pkt(sock, done_hdr, addr, connected)
                return sent_bytes
            ...
            try:
                ...
                # retransmit anything older than timeout
                now = _udpseq_now()
                for s in list(inflight.keys()):
                    last_sent, pkt, tries, plen = inflight[s]
                    if (now - last_sent) >= float(timeout):
                        if tries >= int(retries):
                            raise IOError("udp seq retransmit limit reached (seq=%s)" % s)
                        _udpseq_send_pkt(sock, pkt, addr, connected)
                        inflight[s][0] = now
                        inflight[s][2] = tries + 1
                        last_activity = now
                continue

            if not raw or len(raw) < _UDPSEQ_HDR_LEN:
                continue
            try:
                magic, ver, flags, aseq, totalb = struct.unpack(_UDPSEQ_HDR, raw[:_UDPSEQ_HDR_LEN])
            except Exception:
                continue
            if magic != _UDPSEQ_MAGIC or ver != _UDPSEQ_VER:
                continue
            if flags & _UDPSEQ_FLAG_ACK:
        ...
        while True:
            ...
            if flags & _UDPSEQ_FLAG_DATA:
                ...
                while expected in received:
                    ...
                    expected += 1
                    if on_progress:
                        try: on_progress(expected, None)
                        except Exception: pass

            elif flags & _UDPSEQ_FLAG_DONE:
                done_seen = True
                total_bytes = int(totalb) if totalb else None
                # If no pending packets below expected, and we likely have all, finish soon
                if not received:
                    return True
                # else keep receiving until gaps filled (or timeout)
