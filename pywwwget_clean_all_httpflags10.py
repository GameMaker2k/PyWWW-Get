...
# -*- coding: utf-8 -*-
"""
pywwwget_clean_all_httpflags9.py

Adds save_dir=<path>|auto for TCP auto-save (save=1) on top of httpflags8.

TCP receive flags:
- meta=1         : parse filename+size header
- save=1         : write received bytes to local file
- save_dir=PATH|auto : directory to save into (created if missing). 'auto' chooses a sensible default (Android/Termux friendly).
- overwrite=0|1  : default 0; if 0 uses .1/.2 suffixes to avoid overwrite.

Returns the same file-like object (API unchanged).
"""
...
log = logging.getLogger("pywwwget_all_httpflags9")
if not log.handlers:
...
def _parse_opts(url):
    ...
    opts = {
        ...
        "meta": _gi(q, "meta", 0),
        "save": _gi(q, "save", 0),
        "overwrite": _gi(q, "overwrite", 0),
        "save_dir": _gs(q, "save_dir", None),
    }
    return p, opts
...
# ---- HTTP server + client (same as v8) ----

def send_via_http(fileobj, url):
    # identical to v8; kept small by importing from v8 is not possible here,
    # so reuse the same implementation from v8 (verbatim logic).
    p, o = _parse_opts(url)
...
def _auto_save_dir():
    """
    Best-effort default download directory chooser (Android/Termux friendly).
    Returns a path string or None.
    """
    candidates = [
        # Termux Storage (after termux-setup-storage)
    ...
    for p in candidates:
        try:
            ...
            pass
    # As a last resort, use current directory if writable
    try:
...
def _resolve_save_dir(save_dir):
    ...
    save_dir can be:
    - None -> None
    - 'auto' -> choose a suitable default dir
    ...
    if isinstance(sd, bytes):
        try:
            sd = sd.decode("utf-8", "ignore")
        except Exception:
            pass
    if str(sd).lower() == "auto":
...
def _choose_output_path(base, overwrite, save_dir):
    base = base or "download.bin"
...
def _autosave_fileobj(fileobj, out_path):
    ...
    with open(out_path, "wb") as out:
        shutil.copyfileobj(fileobj, out, length=DEFAULT_CHUNK)
    fileobj.seek(0, 0)
    return out_path
...
def _tcp_send_fileobj(fileobj, url):
    ...
    meta_name = o.get("name") or _safe_filename((p.path or "").strip("/"))
    meta_size = None
    try:
        fileobj.seek(0, 2)
    ...
    try:
            ...
            if len(hdr) == _TCP_META_HDR_LEN:
                try:
                    ...
                    if magic == _TCP_META_MAGIC and ver == _TCP_META_VER:
                        ...
                        meta = {"filename": filename, "size": expect_total, "peer": addr[0] if addr else None}
                        try:
        ...
        if o.get("save"):
            ...
            if meta and meta.get("filename"):
                base = _safe_filename(meta.get("filename"))
            if not base:
                base = _safe_filename((p.path or "").strip("/"))
            out_path = _choose_output_path(base or "download.bin", bool(o.get("overwrite")), o.get("save_dir"))
            saved = _autosave_fileobj(out, out_path)
