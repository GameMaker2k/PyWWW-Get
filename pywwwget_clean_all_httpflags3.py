...
# -*- coding: utf-8 -*-
"""
pywwwget_clean_all_httpflags3.py

Adds:
- print_url=1 (default 1): prints the URL it is serving when the HTTP/HTTPS server starts
- port=0 auto-pick: if port is 0 (or URL port is 0), the OS chooses a free port and we print it

This builds on pywwwget_clean_all_httpflags2.py (allow_net + self-signed https).
...
log = logging.getLogger("pywwwget_all_httpflags3")
if not log.handlers:
...
def _pretty_urls(scheme, bind, port, path, query):
    # If binding to 0.0.0.0, print a couple useful LAN candidates.
    urls = []
    host_for_print = bind
    if bind == "0.0.0.0":
        # best effort: get a likely LAN IP by connecting to a public IP (no traffic actually sent)
        try:
...
def send_via_http(fileobj, url):
    ...
    # - if URL has a port, it wins (even if 0)
    # - else use opts["port"] (may be 0)
    port = p.port if (p.port is not None) else (o["port"] or 8000)
    if port is None:
        port = 8000
    ...
    # Determine file length (best effort) for Range
    try:
        cur = fileobj.tell()
    except Exception:
        cur = None
    ...
    # Resolve actual port when port=0
    ...
    if o["print_url"]:
        ...
        for u in _pretty_urls(served_scheme, bind, actual_port, path, query):
            try:
