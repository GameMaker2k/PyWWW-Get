...
# Optional Bluetooth fallback (PyBluez)
try:
...
def download_file_from_http_file(url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, resumefile=None, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="GET", postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60, returnstats=False):
    """Backward-compatible thin delegator to the HTTP handler engine."""
    return _WWWGET_CLIENT.download_file(
        url,
        headers=headers,
        usehttp=usehttp,
...
def download_file_from_http_bytes(url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, resumefile=None, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="GET", postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60, returnstats=False):
    """Backward-compatible thin delegator to the HTTP handler engine."""
    return _WWWGET_CLIENT.download_bytes(
        url,
        headers=headers,
        usehttp=usehttp,
        usesslcert=usesslcert,
...
def download_file_from_http_to_file(url, outfile, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, resumefile=None, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="GET", postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60, returnstats=False):
    """Backward-compatible thin delegator to the HTTP handler engine."""
    return _WWWGET_CLIENT.download_to_file(
        url,
        outfile,
        headers=headers,
        usehttp=usehttp,
...
def upload_file_to_http_file(infiles, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, postdata=None, jsonpost=False, putfile=None, timeout=60, returnstats=False):
    """Backward-compatible thin delegator to the HTTP handler engine (multipart/form upload)."""
    return _WWWGET_CLIENT.upload_file(
        infiles,
        url,
        headers=headers,
        usehttp=usehttp,
...
def upload_file_to_http_from_file(infile, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, postdata=None, jsonpost=False, putfile=None, timeout=60, returnstats=False):
    """Backward-compatible thin delegator to the HTTP handler engine (upload file path)."""
    return _WWWGET_CLIENT.upload_from_file(
        infile,
        url,
        headers=headers,
        usehttp=usehttp,
...
def upload_file_to_http_bytes(data, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="PUT", timeout=60, returnstats=False):
    """Upload raw bytes to an HTTP/HTTPS URL (defaults to PUT)."""
    return _WWWGET_CLIENT.upload_bytes(
        data,
        url,
        headers=headers,
        usehttp=usehttp,
...
def _bt_socket_stream():
    """Create an RFCOMM stream socket using stdlib (Linux/BlueZ) or PyBluez fallback."""
    try:
        if hasattr(socket, "AF_BLUETOOTH") and hasattr(socket, "BTPROTO_RFCOMM"):
...
def _norm_bt_addr(addr):
    # Accept AA:BB:.. or AA-BB-.. ; allow empty/any markers.
    if not addr:
...
def _bt_bind_addr(addr):
    """Map our normalized BDADDR_ANY marker to what the underlying stack expects."""
    a = _norm_bt_addr(addr)
    # For stdlib BlueZ, BDADDR_ANY is "00:.."; for PyBluez, empty string is typical.
    if a == "00:00:00:00:00:00" and _pybluez is not None and not (hasattr(socket, "AF_BLUETOOTH") and hasattr(socket, "BTPROTO_RFCOMM")):
        return ""
    return a

def _split_bt_netloc(netloc):
    """
...
def _bt_host_channel_from_url(parts, qs, o):
...
def recv_to_fileobj(fileobj, host, port, proto="tcp", path_text=None, **kwargs):
    ...
    if proto in _BT_SCHEMES:
        if not _has_rfcomm():
            try:
                _net_log(kwargs.get("verbose"), "RFCOMM not supported by this Python/socket build")
            except Exception:
                pass
            return False
        ...
        if kwargs.get("print_url"):
            path = path_text or "/"
            try:
                sys.stdout.write("Listening: bt://%s:%d%s\n" % (_norm_bt_addr(host or "00:00:00:00:00:00"), ch, path))
                sys.stdout.flush()
            except Exception:
                pass
        ...
        # The rest matches the TCP stream receiver semantics.
    ...
    if proto in _BT_SCHEMES:
        ...
        if wait_timeout is not None:
            try:
                ...
                wait_timeout = None
        start_t = time.time()
        hostn = _norm_bt_addr(host)
        if hostn == "00:00:00:00:00:00":
            try:
                sock.close()
            except Exception:
                pass
        ...
        while True:
            try:
                ...
                if wait_timeout is not None and wait_timeout >= 0 and (time.time() - start_t) >= wait_timeout:
                    ...
                    return False
                try:
                    _net_log(kwargs.get("verbose"), "BT: waiting for receiver (connect refused), retrying...")
                    time.sleep(0.1)
                except Exception:
                    pass
                continue
...
# Centralized dependency detection (best-of: optiplexed style)
_DEPENDENCIES: Dict[str, bool] = {}
for _name in ("requests", "httpx", "urllib3", "httpcore", "pycurl", "mechanize", "pysftp", "paramiko"):
    try:
        __import__(_name)
        _DEPENDENCIES[_name] = True
    except Exception:
        _DEPENDENCIES[_name] = False

def _best_http_backend(preferred: Optional[str] = None) -> str:
    """Pick the best available HTTP backend."""
    if preferred and _DEPENDENCIES.get(preferred, False):
        return preferred
    for cand in ("httpx", "requests"):
        if _DEPENDENCIES.get(cand, False):
            return cand
    return "urllib"

class HTTPHandlerV2:
    """
    Unified HTTP handler that supports:
      - backends: httpx / requests / urllib (fallback)
      - methods: GET/HEAD/POST/PUT/PATCH/DELETE
      - resume via Range (when resumefile is provided and seekable)
      - basic auth via URL user:pass
      - optional cookie jar persistence (cookiefile path)
      - returnstats mode (returns {'file': fp, 'status': ..., ...})
    """
    def __init__(self, backend: Optional[str] = None):
        self.backend = _best_http_backend(backend)
        self._session = None
        self._client = None

    def _strip_auth_url(self, url: str) -> Tuple[str, Optional[Tuple[str,str]]]:
        p = urlparse(url)
        auth = None
        if p.username and p.password:
            auth = (unquote(p.username), unquote(p.password))
    ...
    def _normalize_headers(self, headers: Any) -> Dict[str, str]:
        if headers is None:
            return {}
        if isinstance(headers, list):
            # accept list of tuples or "Key: Value" strings
            try:
                return make_http_headers_from_list_to_dict(headers)
            except Exception:
                out = {}
                for it in headers:
    ...
    def _apply_common(self, headers: Dict[str,str], httpuseragent: Optional[str], httpreferer: Optional[str]) -> Dict[str,str]:
        h = dict(headers or {})
        if httpuseragent:
            h["User-Agent"] = httpuseragent
        elif "User-Agent" not in h and "user-agent" not in {k.lower():k for k in h}.keys():
            try:
                h["User-Agent"] = pywwwgetUserAgent
            except Exception:
                pass
        if httpreferer and "Referer" not in h:
            h["Referer"] = httpreferer
        return h

    def _apply_resume(self, headers: Dict[str,str], resumefile: Any) -> Tuple[Dict[str,str], int]:
        resume_off = 0
        if resumefile is not None and hasattr(resumefile, "seek") and hasattr(resumefile, "tell"):
            try:
                resumefile.seek(0, 2)
                resume_off = int(resumefile.tell())
                if resume_off > 0:
                    headers = dict(headers)
                    headers["Range"] = f"bytes={resume_off}-"
            except Exception:
                resume_off = 0
        return headers, resume_off
    ...
    def download(self, url: str, *,
                 ...
                 httpcookie: Optional[str] = None,
                 returnstats: bool = False) -> Union[BinaryIO, Dict, bool]:
        method = (httpmethod or "GET").upper()
        clean_url, auth = self._strip_auth_url(url)
        h = self._normalize_headers(headers)
        h = self._apply_common(h, httpuseragent, httpreferer)
        h, resume_off = self._apply_resume(h, resumefile)

        # Cookie support: best effort. If httpcookie is a path, use MozillaCookieJar/LWPCookieJar like legacy.
        cookiejar = None
        cookiefile = None
        if httpcookie:
            cookiefile = httpcookie
            try:
                cookie_name, cookie_ext = os.path.splitext(cookiefile)
                policy = cookielib.DefaultCookiePolicy(netscape=True, rfc2965=False, hide_cookie2=True)
                if cookie_ext == ".lwp":
                    cookiejar = cookielib.LWPCookieJar(cookiefile, policy=policy)
                else:
                    cookiejar = cookielib.MozillaCookieJar(cookiefile, policy=policy)
                if os.path.exists(cookiefile):
                    cookiejar.load(ignore_discard=True, ignore_expires=True)
            except Exception:
                cookiejar = None

        verify = usesslcert if usesslcert is not None else defcert

        # Decide output stream
        outfp = resumefile if (resumefile is not None and hasattr(resumefile, "write")) else MkTempFile()
        if outfp is resumefile:
            try:
                outfp.seek(0, 2)
            except Exception:
                pass
        ...
        try:
            if self.backend == "requests" and _DEPENDENCIES.get("requests"):
                import requests
                if self._session is None or not keepsession:
                ...
                if putfile is not None:
                    # requests supports file-like for data
                    data = putfile
                ...
                if cookiejar is not None and cookiefile:
                    try:
                ...
                if returnstats:
                    return {
                        ...
                        "url": getattr(resp, "url", clean_url),
                        "bytes": int(outfp.tell()) if hasattr(outfp, "tell") else None,
                        "resume_offset": resume_off,
                        "backend": "requests",
                    }
                return outfp

            if self.backend == "httpx" and _DEPENDENCIES.get("httpx"):
                import httpx
                if self._client is None or not keepsession:
                    self._client = httpx.Client(follow_redirects=True, verify=verify, timeout=timeout)
                ...
                if sendfiles is not None:
                    files = sendfiles
                if putfile is not None:
                    content = putfile.read() if hasattr(putfile, "read") else putfile
                elif postdata is not None:
                    if jsonpost:
                        json_data = postdata
                    else:
                        content = postdata
                r = self._client.request(method, clean_url, headers=h, auth=auth, content=content, json=json_data, files=files, cookies=cookies)
                r.raise_for_status()
                ...
                if returnstats:
                    return {
                        ...
                        "reason": http_status_to_reason(int(r.status_code)),
            ...
            # urllib fallback (supports GET/POST/PUT minimally)
            req_data = None
            if putfile is not None:
                req_data = putfile.read() if hasattr(putfile, "read") else putfile
            elif postdata is not None:
                if jsonpost:
                    req_data = json.dumps(postdata).encode("utf-8")
                    h.setdefault("Content-Type", "application/json")
                else:
                    if isinstance(postdata, (bytes, bytearray)):
                        req_data = bytes(postdata)
                    else:
                        req_data = str(postdata).encode("utf-8")
            req = Request(clean_url, data=req_data, headers=h, method=method)
            # cookie handling with urllib: install opener for this call
            opener = None
            if cookiejar is not None:
                opener = build_opener(HTTPCookieProcessor(cookiejar))
            resp = (opener.open(req, timeout=timeout) if opener else urlopen(req, timeout=timeout))
            status = getattr(resp, "status", None) or getattr(resp, "code", None) or 200
            outfp.write(resp.read())
            ...
            if returnstats:
                return {
                    "file": outfp,
                    "status": int(status),
                    "reason": http_status_to_reason(int(status)),
            ...
            _LOG.error(f"HTTPHandlerV2 failed ({self.backend}): {e}")
            return False

# Preserve legacy HTTP function (from original optimized module) and override with V2.
try:
    download_file_from_http_file_legacy = download_file_from_http_file  # type: ignore[name-defined]
except Exception:
    download_file_from_http_file_legacy = None

def download_file_from_http_file(url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert,
                                 resumefile=None, keepsession=False, insessionvar=None,
                                 httpuseragent=None, httpreferer=None, httpcookie=None,
                                 httpmethod="GET", postdata=None, jsonpost=False,
                                 sendfiles=None, putfile=None, timeout=60, returnstats=False):
    """
    Best-of-both combined HTTP downloader.

    - Uses improved requests/httpx/urllib implementation with better logging + consistent stats.
    - Falls back to legacy implementation for exotic backends (pycurl/mechanize/httpcore/urllib3) or
      when V2 fails and legacy exists.
    """
    backend = usehttp
    if backend in ("pycurl", "mechanize", "httpcore", "urllib3"):
        if callable(download_file_from_http_file_legacy):
            return download_file_from_http_file_legacy(
                url, headers=headers, usehttp=usehttp, usesslcert=usesslcert,
                resumefile=resumefile, keepsession=keepsession, insessionvar=insessionvar,
                httpuseragent=httpuseragent, httpreferer=httpreferer, httpcookie=httpcookie,
                httpmethod=httpmethod, postdata=postdata, jsonpost=jsonpost, sendfiles=sendfiles,
                putfile=putfile, timeout=timeout, returnstats=returnstats
            )
    handler = HTTPHandlerV2(backend=backend)
    res = handler.download(
        url,
        headers=headers,
        usesslcert=usesslcert,
        timeout=timeout,
    ...
    if res is False and callable(download_file_from_http_file_legacy):
        _LOG.info("Falling back to legacy HTTP downloader.")
        return download_file_from_http_file_legacy(
            url, headers=headers, usehttp=usehttp, usesslcert=usesslcert,
            resumefile=resumefile, keepsession=keepsession, insessionvar=insessionvar,
            httpuseragent=httpuseragent, httpreferer=httpreferer, httpcookie=httpcookie,
...
# ============================================================
# Refactored "handler" architecture (optiplexed-style)
# - Keeps full backward compatibility by:
#   * preserving legacy protocol functions
#   * overriding the *_internet_* router functions to use handlers
#   * leaving public per-protocol functions intact
# ============================================================
...
_logger = logging.getLogger("pywwwget")
if not _logger.handlers:
    # don't force a logging configuration; just set a sane default level
...
def _parse_url_parts(url: str) -> URLParts:
    p = urlparse(url)
    return URLParts(
...
class TransferHandler:
    ...
    The handler methods intentionally mirror the existing public API function
    signatures, so wrappers stay compatible.
    """
    schemes: Tuple[str, ...] = ()
    ...
    # File-like return (legacy behavior): returns file object or False
    def download_file(self, url: str, **kwargs):
        raise NotImplementedError

    # Bytes return (legacy behavior): returns bytes or False
    def download_bytes(self, url: str, **kwargs):
        raise NotImplementedError

    # Download to a target path (legacy behavior): returns outfile or False
    def download_to_file(self, url: str, outfile: str, **kwargs):
        raise NotImplementedError

    # Upload a file-like object (legacy behavior): returns something truthy or False
    def upload_file(self, fileobj, url: str, **kwargs):
        raise NotImplementedError

    def upload_bytes(self, data: bytes, url: str, **kwargs):
        raise NotImplementedError

    def upload_from_file(self, infile: str, url: str, **kwargs):
        raise NotImplementedError


class HTTPHandler(TransferHandler):
    schemes = ("http", "https")


    # ---- private engine methods (migrated from module-level functions) ----
    def _download_file_engine(self, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, resumefile=None, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="GET", postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60, returnstats=False):
        if headers is None:
            headers = {}
        else:
            if(isinstance(headers, list)):
    ...
    def _download_bytes_engine(self, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, resumefile=None, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="GET", postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60, returnstats=False):
    ...
    def _download_to_file_engine(self, url, outfile, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, postdata=None, jsonpost=False, sendfiles=None, putfile=None, timeout=60):
        if(os.path.exists(outfile)):
            outfile = open(outfile, "ab")
            outfile.seek(0, 2)
    ...
    def _upload_file_engine(self, infiles, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, postdata=None, jsonpost=False, putfile=None, timeout=60, returnstats=False):
    ...
    def _upload_from_file_engine(self, infiles, infields, url, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, postdata=None, jsonpost=False, putfile=None, timeout=60, returnstats=False):
    ...
    def _upload_bytes_engine(self, data: bytes, url: str, headers=None, usehttp=__use_http_lib__, usesslcert=defcert, keepsession=False, insessionvar=None, httpuseragent=None, httpreferer=None, httpcookie=None, httpmethod="PUT", timeout=60, returnstats=False):
        # Use PUT by default; callers can pass httpmethod="POST" if needed.
        buf = io.BytesIO(data if data is not None else b"")
        buf.seek(0)
        return self._download_file_engine(
            url,
            headers=headers,
            usehttp=usehttp,
            usesslcert=usesslcert,
    ...
    # ---- public handler interface (used by WWWGetClient) ----
    def download_file(self, url: str, **kwargs):
        return self._download_file_engine(url, **kwargs)

    def download_bytes(self, url: str, **kwargs):
        return self._download_bytes_engine(url, **kwargs)

    def download_to_file(self, url: str, outfile: str, **kwargs):
        return self._download_to_file_engine(url, outfile, **kwargs)

    def upload_file(self, fileobj, url: str, **kwargs):
        return self._upload_file_engine(fileobj, url, **kwargs)

    def upload_bytes(self, data: bytes, url: str, **kwargs):
        return self._upload_bytes_engine(data, url, **kwargs)

    def upload_from_file(self, infile: str, url: str, **kwargs):
        return self._upload_from_file_engine(infile, url, **kwargs)



class FTPHandler(TransferHandler):
    schemes = ("ftp", "ftps")

    # ---- private engine methods (migrated from legacy functions) ----
    def _login(self, ftp, user, pw):
        if user is None:
            user = "anonymous"
        if pw is None:
            pw = "anonymous" if user == "anonymous" else ""
    ...
    def _detect_cwd(self, ftp, file_dir):
        if not file_dir:
            return False
        try:
            ftp.cwd(file_dir)
    ...
    def _download_file_engine(self, url, resumefile=None, timeout=60, returnstats=False):
        p = urlparse(url)
        if p.scheme not in ("ftp", "ftps"):
            return False
        if p.scheme == "ftps" and not ftpssl:
        ...
        try:
            ...
            use_cwd = self._detect_cwd(ftp, file_dir)
            retr_path = os.path.basename(path) if use_cwd else path
            ...
            if resumefile is not None and hasattr(resumefile, "write"):
                try:
                    ...
                    pass
                bio = resumefile
                # NOTE: original code prepared 'rest' but didn't pass it to retrbinary;
                # keep behavior (resume is handled by appending to file-like object).
            else:
                bio = MkTempFile()
            ...
            if returnstats:
                ...
                return {
                    ...
                    'ContentsizeAlt': {
                        'IEC': get_readable_size(fulldatasize, 2, "IEC"),
                        'SI': get_readable_size(fulldatasize, 2, "SI")
                    },
                    'Headers': None,
                    'Version': None,
                    'Method': None,
                    ...
                    'URL': url,
                    'Code': None,
                    'RequestTime': {'StartTime': start_time, 'EndTime': end_time, 'TotalTime': total_time},
                    'FTPLib': 'pyftp',
                }
            return bio
    ...
    def _download_bytes_engine(self, url, resumefile=None, timeout=60, returnstats=False):
        fp = self._download_file_engine(url, resumefile=resumefile, timeout=timeout, returnstats=returnstats)
        return fp.read() if fp else False

    def _download_to_file_engine(self, url, outfile, timeout=60):
        # keep legacy behavior: append if exists
        if os.path.exists(outfile):
            with open(outfile, "ab") as outfp:
                outfp.seek(0, 2)
                ok = self._download_file_engine(url, resumefile=outfp, timeout=timeout, returnstats=False)
                return bool(ok)
        else:
            data = self._download_bytes_engine(url, resumefile=None, timeout=timeout, returnstats=False)
            if data is False:
                return False
    ...
    def _upload_file_engine(self, fileobj, url, timeout=60):
        p = urlparse(url)
        if p.scheme not in ("ftp", "ftps"):
            return False
        if p.scheme == "ftps" and not ftpssl:
    ...
    def _upload_bytes_engine(self, data, url, timeout=60):
        bio = MkTempFile(_to_bytes(data))
        try:
            return self._upload_file_engine(bio, url, timeout=timeout)
        finally:
            try:
                bio.close()
            except Exception:
    ...
    def _upload_from_file_engine(self, infile, url, timeout=60):
        with open(infile, "rb") as f:
            ok = self._upload_file_engine(f, url, timeout=timeout)
        return bool(ok)
...
class SFTPHandler(TransferHandler):
    schemes = ("sftp", "scp")

    def _prefer_pysftp(self) -> bool:
        try:
            return bool(globals().get("__use_pysftp__")) and bool(globals().get("havepysftp"))
        except Exception:
            return False

    def _detect_cwd(self, sftp, file_dir):
        if not file_dir:
            return False
        try:
            sftp.chdir(file_dir)
    ...
    # ---- paramiko engines ----
    def _download_paramiko_engine(self, url, timeout=60, returnstats=False):
        if not haveparamiko:
            return False
        p = urlparse(url)
        if p.scheme not in ("sftp", "scp"):
        ...
        try:
            ...
            file_dir = os.path.dirname(path)
            use_cwd = self._detect_cwd(sftp, file_dir)
            retr_path = os.path.basename(path) if use_cwd else path
    ...
    def _upload_paramiko_engine(self, fileobj, url, timeout=60):
        if not haveparamiko:
            return False
        p = urlparse(url)
        if p.scheme not in ("sftp", "scp"):
    ...
    # ---- pysftp engines ----
    def _download_pysftp_engine(self, url, timeout=60, returnstats=False):
        if not havepysftp:
            return False
        p = urlparse(url)
        if p.scheme not in ("sftp", "scp"):
    ...
    def _upload_pysftp_engine(self, fileobj, url, timeout=60):
        if not havepysftp:
            return False
        p = urlparse(url)
        if p.scheme not in ("sftp", "scp"):
    ...
    # ---- unified engines ----
    def _download_file_engine(self, url, timeout=60, returnstats=False):
        if self._prefer_pysftp():
            return self._download_pysftp_engine(url, timeout=timeout, returnstats=returnstats)
        return self._download_paramiko_engine(url, timeout=timeout, returnstats=returnstats)

    def _download_bytes_engine(self, url, timeout=60, returnstats=False):
        fp = self._download_file_engine(url, timeout=timeout, returnstats=returnstats)
        return fp.read() if fp else False

    def _download_to_file_engine(self, url, outfile, timeout=60):
        data = self._download_bytes_engine(url, timeout=timeout, returnstats=False)
        if data is False:
            return False
        with open(outfile, "wb") as outfp:
            outfp.write(data)
    ...
    def _upload_file_engine(self, fileobj, url, timeout=60):
        if self._prefer_pysftp():
            return self._upload_pysftp_engine(fileobj, url, timeout=timeout)
        return self._upload_paramiko_engine(fileobj, url, timeout=timeout)
...
class TFTPHandler(TransferHandler):
    schemes = ("tftp",)
    ...
    def _download_file_engine(self, url, timeout=60, returnstats=False):
        ...
        socket.setdefaulttimeout(float(timeout))
        try:
            bio = tftp_download(host, path, port, timeout=float(timeout))
            fulldatasize = bio.tell()
            try:
                bio.seek(0, 0)
            except Exception:
    ...
    def _upload_file_engine(self, fileobj, url, timeout=60):
        ...
        try:
            try:
                ...
                pass
            tftp_upload(host, path, fileobj, port, timeout=float(timeout))
            try:
                fileobj.seek(0, 0)
            except Exception:
                pass
...
class DataHandler(TransferHandler):
    schemes = ("data",)
    ...
    def upload_bytes(self, data: bytes, url: str, **kwargs):
        return data_url_encode(BytesIO(data))

    def upload_from_file(self, infile: str, url: str, **kwargs):
        with open(infile, "rb") as f:
            return data_url_encode(f)


class FileHandler(TransferHandler):
    schemes = ("file", "")
    ...
    def download_bytes(self, url: str, **kwargs):
        with self.download_file(url, **kwargs) as f:
            return f.read()

    def download_to_file(self, url: str, outfile: str, **kwargs):
        with self.download_file(url, **kwargs) as src, open(outfile, "wb") as dst:
    ...
    def upload_bytes(self, data: bytes, url: str, **kwargs):
        return self.upload_file(BytesIO(data), url, **kwargs)

    def upload_from_file(self, infile: str, url: str, **kwargs):
        with open(infile, "rb") as f:
            return self.upload_file(f, url, **kwargs)


class BluetoothHandler(TransferHandler):
    """Bluetooth RFCOMM (bt://, rfcomm://, bluetooth://) via stdlib BlueZ or PyBluez."""

    schemes = _BT_SCHEMES

    def _parse_bt(self, url: str):
        parts = _parse_url_parts(url)
        qs = parse_qs(parts.query or "")
        _path = parts.path or "/"
        # Reuse the net option parser so ?timeout= etc work consistently.
        _p2, o = _parse_net_url(url)
        addr, ch = _bt_host_channel_from_url(parts, qs, o)
        return parts, o, addr, ch, _path

    def download_file(self, url: str, **kwargs):
        parts, o, bind_addr, ch, path = self._parse_bt(url)
        outfile = MkTempFile()
        ok = recv_to_fileobj(outfile, host=bind_addr, port=ch, proto="bt", path_text=path, **o)
        if not ok:
            try:
                outfile.close()
            except Exception:
    ...
    def download_bytes(self, url: str, **kwargs):
        f = self.download_file(url, **kwargs)
        if not f:
            return False
        try:
            data = f.read()
    ...
    def upload_file(self, fileobj, url: str, **kwargs):
        parts, o, addr, ch, path = self._parse_bt(url)
        try:
            fileobj.seek(0, 0)
        except Exception:
            pass
        return send_from_fileobj(fileobj, host=addr, port=ch, proto="bt", path_text=path, **o)
...
class SocketHandler(TransferHandler):
    schemes = ("tcp", "udp")
    ...
    def download_file(self, url: str, **kwargs):
        ...
        outfile = MkTempFile()
        ok = recv_to_fileobj(
            outfile, host=host, port=port, proto=parts.scheme,
            mode=o.get("mode"), timeout=o.get("timeout")
        )
        if not ok:
            try:
                outfile.close()
    ...
    def upload_file(self, fileobj, url: str, **kwargs):
        ...
        return send_from_fileobj(
            fileobj, host=host, port=port, proto=parts.scheme,
            mode=o.get("mode"), timeout=o.get("timeout")
        )
...
class WWWGetClient:
    """Clean router around per-protocol handlers."""

    def __init__(self, handlers: Optional[Tuple[TransferHandler, ...]] = None):
        if handlers is None:
            handlers = (
                HTTPHandler(),
                FTPHandler(),
        ...
        self._handlers: Tuple[TransferHandler, ...] = handlers
        self._scheme_map: Dict[str, TransferHandler] = {}
        for h in handlers:
            for s in getattr(h, "schemes", ()):
                self._scheme_map[s.lower()] = h

    def handler_for(self, url: str) -> Optional[TransferHandler]:
        scheme = _parse_url_parts(url).scheme
        return self._scheme_map.get(scheme)

    def download_file(self, url: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.download_file(url, **kwargs)

    def download_bytes(self, url: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.download_bytes(url, **kwargs)

    def download_to_file(self, url: str, outfile: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.download_to_file(url, outfile, **kwargs)

    def upload_file(self, fileobj, url: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.upload_file(fileobj, url, **kwargs)

    def upload_bytes(self, data: bytes, url: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.upload_bytes(data, url, **kwargs)

    def upload_from_file(self, infile: str, url: str, **kwargs):
        h = self.handler_for(url)
        if not h:
            return False
        return h.upload_from_file(infile, url, **kwargs)


# Global singleton used by the compatibility router functions below
_WWWGET_CLIENT = WWWGetClient()


# ---------------------------
# Compatibility routers (override legacy definitions)
# ---------------------------

def download_file_from_internet_file(url, **kwargs):
    """Download to a file-like object (or False). Backward compatible."""
    try:
        return _WWWGET_CLIENT.download_file(url, **kwargs)
    except Exception as e:
        _logger.exception("download_file_from_internet_file failed: %s", e)
        return False


def download_file_from_internet_bytes(url, **kwargs):
    """Download to bytes (or False). Backward compatible."""
    try:
        return _WWWGET_CLIENT.download_bytes(url, **kwargs)
    except Exception as e:
        _logger.exception("download_file_from_internet_bytes failed: %s", e)
        return False


def download_file_from_internet_to_file(url, outfile, **kwargs):
    """Download to a destination path (or False). Backward compatible."""
    try:
        return _WWWGET_CLIENT.download_to_file(url, outfile, **kwargs)
    except Exception as e:
        _logger.exception("download_file_from_internet_to_file failed: %s", e)
        return False


def upload_file_to_internet_file(fileobj, url, **kwargs):
    """Upload a file-like object. Backward compatible."""
    try:
        return _WWWGET_CLIENT.upload_file(fileobj, url, **kwargs)
    except Exception as e:
        _logger.exception("upload_file_to_internet_file failed: %s", e)
        return False


def upload_file_to_internet_bytes(data, url, **kwargs):
    """Upload raw bytes. Backward compatible."""
    try:
        if isinstance(data, (bytes, bytearray)):
            return _WWWGET_CLIENT.upload_bytes(bytes(data), url, **kwargs)
        return upload_file_to_internet_file(data, url, **kwargs)
    except Exception as e:
        _logger.exception("upload_file_to_internet_bytes failed: %s", e)
        return False


def upload_file_to_internet_from_file(infile, url, **kwargs):
    """Upload a local file to a URL. Backward compatible."""
    try:
        return _WWWGET_CLIENT.upload_from_file(infile, url, **kwargs)
    except Exception as e:
        _logger.exception("upload_file_to_internet_from_file failed: %s", e)
        return False


# ---------------------------
# Protocol-level thin delegators (override legacy definitions)
# ---------------------------

# FTP / FTPS
def download_file_from_ftp_file(url, resumefile=None, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["ftp"]._download_file_engine(url, resumefile=resumefile, timeout=timeout, returnstats=returnstats)

def download_file_from_ftp_bytes(url, resumefile=None, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["ftp"]._download_bytes_engine(url, resumefile=resumefile, timeout=timeout, returnstats=returnstats)

def download_file_from_ftp_to_file(url, outfile, timeout=60):
    return _WWWGET_CLIENT._scheme_map["ftp"]._download_to_file_engine(url, outfile, timeout=timeout)

def download_file_from_ftps_file(url, resumefile=None, timeout=60, returnstats=False):
    return download_file_from_ftp_file(url, resumefile=resumefile, timeout=timeout, returnstats=returnstats)

def download_file_from_ftps_bytes(url, resumefile=None, timeout=60, returnstats=False):
    return download_file_from_ftp_bytes(url, resumefile=resumefile, timeout=timeout, returnstats=returnstats)

def download_file_from_ftps_to_file(url, outfile, timeout=60):
    return download_file_from_ftp_to_file(url, outfile, timeout=timeout)

def upload_file_to_ftp_file(fileobj, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["ftp"]._upload_file_engine(fileobj, url, timeout=timeout)

def upload_file_to_ftp_bytes(data, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["ftp"]._upload_bytes_engine(data, url, timeout=timeout)

def upload_file_to_ftp_from_file(infile, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["ftp"]._upload_from_file_engine(infile, url, timeout=timeout)

def upload_file_to_ftps_file(fileobj, url, timeout=60):
    return upload_file_to_ftp_file(fileobj, url, timeout=timeout)

def upload_file_to_ftps_bytes(data, url, timeout=60):
    return upload_file_to_ftp_bytes(data, url, timeout=timeout)

def upload_file_to_ftps_from_file(infile, url, timeout=60):
    return upload_file_to_ftp_from_file(infile, url, timeout=timeout)


# SFTP / SCP (paramiko or pysftp)
def download_file_from_sftp_file(url, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["sftp"]._download_file_engine(url, timeout=timeout, returnstats=returnstats)

def download_file_from_sftp_bytes(url, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["sftp"]._download_bytes_engine(url, timeout=timeout, returnstats=returnstats)

def download_file_from_sftp_to_file(url, outfile, timeout=60):
    return _WWWGET_CLIENT._scheme_map["sftp"]._download_to_file_engine(url, outfile, timeout=timeout)

def upload_file_to_sftp_file(fileobj, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["sftp"]._upload_file_engine(fileobj, url, timeout=timeout)

def upload_file_to_sftp_bytes(data, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["sftp"]._upload_bytes_engine(data, url, timeout=timeout)

def upload_file_to_sftp_from_file(infile, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["sftp"]._upload_from_file_engine(infile, url, timeout=timeout)

# pysftp explicit functions remain available for compatibility;
# they now route through the same handler engines (when pysftp is enabled).
def download_file_from_pysftp_file(url, timeout=60, returnstats=False):
    # force pysftp path
    h = _WWWGET_CLIENT._scheme_map["sftp"]
    try:
        return h._download_pysftp_engine(url, timeout=timeout, returnstats=returnstats)
    except Exception:
        return False

def download_file_from_pysftp_bytes(url, timeout=60, returnstats=False):
    fp = download_file_from_pysftp_file(url, timeout=timeout, returnstats=returnstats)
    return fp.read() if fp else False

def download_file_from_pysftp_to_file(url, outfile, timeout=60):
    data = download_file_from_pysftp_bytes(url, timeout=timeout, returnstats=False)
    if data is False:
        return False
    with open(outfile, "wb") as outfp:
        outfp.write(data)
...
def upload_file_to_pysftp_file(fileobj, url, timeout=60):
    h = _WWWGET_CLIENT._scheme_map["sftp"]
    try:
        return h._upload_pysftp_engine(fileobj, url, timeout=timeout)
    except Exception:
        return False

def upload_file_to_pysftp_bytes(data, url, timeout=60):
    return upload_file_to_pysftp_file(MkTempFile(_to_bytes(data)), url, timeout=timeout)

def upload_file_to_pysftp_from_file(infile, url, timeout=60):
    with open(infile, "rb") as f:
        return upload_file_to_pysftp_file(f, url, timeout=timeout)


# TFTP
def download_file_from_tftp_file(url, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["tftp"]._download_file_engine(url, timeout=timeout, returnstats=returnstats)

def download_file_from_tftp_bytes(url, timeout=60, returnstats=False):
    return _WWWGET_CLIENT._scheme_map["tftp"]._download_bytes_engine(url, timeout=timeout, returnstats=returnstats)

def download_file_from_tftp_to_file(url, outfile, timeout=60):
    return _WWWGET_CLIENT._scheme_map["tftp"]._download_to_file_engine(url, outfile, timeout=timeout)

def upload_file_to_tftp_file(fileobj, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["tftp"]._upload_file_engine(fileobj, url, timeout=timeout)

def upload_file_to_tftp_bytes(data, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["tftp"]._upload_bytes_engine(data, url, timeout=timeout)

def upload_file_to_tftp_from_file(infile, url, timeout=60):
    return _WWWGET_CLIENT._scheme_map["tftp"]._upload_from_file_engine(infile, url, timeout=timeout)
