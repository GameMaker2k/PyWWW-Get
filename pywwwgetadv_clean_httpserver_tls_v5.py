...
if framing == "len":
    # Read fixed header: magic + uint64 size.
    # If header is missing/mismatched, fall back to legacy streaming.
    try:
        hdr = b""
        while len(hdr) < 12:
            part = conn.recv(12 - len(hdr))
            if not part:
                break
            hdr += part
        if len(hdr) == 12 and hdr[:4] == b"PWG1":
            remaining = struct.unpack("!Q", hdr[4:12])[0]
            ...
            # Not our header: write any bytes we pulled and continue legacy mode
            if hdr:
                fileobj.write(hdr)
    except Exception:
        pass
...
if framing == "len":
    ...
    if size is None or size < 0:
        size = 0
    try:
        hdr = b"PWG1" + struct.pack("!Q", int(size))
        sock.sendall(hdr)
    except Exception:
        pass
    ...
    while True:
        ...
        try:
            ...
            if total_bytes is not None and total_bytes > 0:
                try:
                    target = int(total_bytes - (resume_off if resume_off > 0 else 0))
                    if target < 0:
        ...
        if flags & _UF_META:
            # META: record total length when provided
            try:
                if _t and int(_t) > 0:
                    total_bytes = int(_t)
            except Exception:
        ...
        if seq == expected:
            ...
            while expected in received:
                _p = received.pop(expected)
                fileobj.write(_p)
                written += len(_p)
                expected += 1

# If total length is known, we can finish immediately once received enough bytes
framing = (kwargs.get("framing") or "").lower()
if total_bytes is not None and total_bytes > 0:
    try:
        # For resume, sender started at resume_off; our written counts from there.
        target = int(total_bytes - (resume_off if resume_off > 0 else 0))
        if target < 0:
            target = int(total_bytes)
    except Exception:
...
def upload_file_to_internet_file(fileobj, url):
    ...
    if p.scheme in ("tcp", "udp"):
        ...
        ok = send_from_fileobj(
            ...
            done=o.get("done"), done_token=o.get("done_token")
        )
