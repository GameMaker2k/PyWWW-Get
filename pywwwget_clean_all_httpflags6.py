...
# -*- coding: utf-8 -*-
"""
pywwwget_clean_all_httpflags6.py

Builds on httpflags5 and improves TCP receive timeouts:

TCP receive (download_file_from_internet_file("tcp://...")):
- accept_timeout=<sec>  : wait for a sender to connect (listening socket timeout)
- read_timeout=<sec>    : per-recv timeout on accepted socket (default: timeout)
- idle_timeout=<sec>    : if >0, finish after this many seconds with no data (instead of stopping on first socket.timeout)
                           default: 0 (keep waiting; only EOF ends transfer)
- timeout=<sec>         : kept for compatibility; used as default for read_timeout if read_timeout not provided

Other flags from httpflags5 remain:
- reuse=1, linger=0|1, print_url=1, port=0
UDP receive:
- end_timeout=<sec> (silence-based completion) and reuse=1, print_url=1, port=0
...
log = logging.getLogger("pywwwget_all_httpflags6")
if not log.handlers:
...
def _parse_opts(url):
    ...
    opts = {
        ...
        "end_timeout": _gf(q, "end_timeout", 2.0),  # UDP silence finish
    }
    return p, opts
...
# ---- HTTP server and client are same as v5 (kept) ----
# For brevity, reusing v5 implementations (unchanged) by importing from v5 is not possible in single file delivery,
# so we keep a minimal but identical subset. (Copy from v5.)
...
def _tcp_send_fileobj(fileobj, url):
    ...
    try:
        ...
        while True:
            ...
            _throttle(o["rate"], sent, started)
            if o["progress"] and (time.time() - last_report) >= max(1, o["progress_every"]):
                sys.stdout.write(_progress_line("Send(tcp)", sent, None) + "\n")
                sys.stdout.flush()
                last_report = time.time()
        if o["progress"]:
            sys.stdout.write(_progress_line("Send(tcp) done", sent, None) + "\n")
            sys.stdout.flush()
        return sent
    finally:
        s.close()
    ...
    try:
        while True:
            try:
                ...
                # If idle_timeout==0, keep waiting (do not end transfer)
                if float(o["idle_timeout"]) <= 0.0:
                    continue
                # Else finish after idle_timeout seconds of no data
            ...
            if o["progress"] and (time.time() - last_report) >= max(1, o["progress_every"]):
                sys.stdout.write(_progress_line("Recv(tcp)", total, None) + "\n")
                sys.stdout.flush()
                last_report = time.time()

        if o["progress"]:
            sys.stdout.write(_progress_line("Recv(tcp) done", total, None) + "\n")
            sys.stdout.flush()
