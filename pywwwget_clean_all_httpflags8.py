...
# -*- coding: utf-8 -*-
"""
pywwwget_clean_all_httpflags8.py

Builds on httpflags7 and adds optional auto-save on TCP receive:

TCP receive (download_file_from_internet_file("tcp://...")):
- meta=1    : (from v7) expect/parse filename + size header
- save=1    : after receiving, write the received bytes to a local file
             - filename defaults to meta filename (if present), else URL path basename, else "download.bin"
             - returns the original file-like object (so API stays the same)
- overwrite=0|1 : if 0 (default), avoids overwriting by appending ".1", ".2", etc.

All previous flags remain.
...
log = logging.getLogger("pywwwget_all_httpflags8")
if not log.handlers:
...
# ---- HTTP server + client (unchanged from v7) ----
...
def _safe_filename(name):
    # very small sanitization: strip paths and keep basename
    if not name:
        return None
    try:
        name = name.replace("\\", "/")
...
def _choose_output_path(base, overwrite):
...
def _tcp_recv_to_temp(url):
    ...
    try:
        ...
        # Auto-save (no API change: still return file-like object)
        if o.get("save"):
            base = None
            ...
            out_path = _choose_output_path(base or "download.bin", bool(o.get("overwrite")))
