...
def send_from_fileobj(fileobj, host, port, proto="tcp", path_text=None, **kwargs):
    ...
    if mode == "raw":
        ...
        # META length for raw UDP (optional but enabled by default)
        # If present, receiver can finish immediately after len bytes (no silence wait).
        raw_meta = kwargs.get("raw_meta", True)
        raw_sha = kwargs.get("raw_sha", False)
        raw_hash = (kwargs.get("raw_hash") or "sha256").lower()
        ...
        # If we need a checksum, compute it over the remaining bytes.
        hexd = None
        if raw_sha:
            try:
                h = hashlib.sha256() if raw_hash != "md5" else hashlib.md5()
                # hash remaining bytes without changing final streaming position
                pos2 = fileobj.tell()
                while True:
        ...
        if raw_sha and hexd:
            try:
                algo = (b"md5" if raw_hash == "md5" else b"sha256")
                sock.sendto(b"HASH " + algo + b" " + hexd.encode("ascii") + b"\n", addr)
            except Exception:
                pass
        ...
        # Still send DONE for compatibility / receivers without META
        try:
...
def _udp_raw_recv(fileobj, host, port, **kwargs):
    ...
    Backwards compatible behavior:
      ...
      - If a HASH <algo> <hexdigest>\n packet is received (and raw_sha=1), verify after receive.
    ...
    raw_sha = bool(kwargs.get("raw_sha", False))
    raw_hash = (kwargs.get("raw_hash") or "sha256").lower()
    expected_hash = None
    expected_algo = None
    ...
    while True:
        ...
        # META packet only honored before data starts
        if expected_len is None and pkt.startswith(b"META "):
            try:
                line = pkt.split(b"\n", 1)[0]
        ...
        # HASH packet only honored before data starts
        if expected_hash is None and pkt.startswith(b"HASH "):
            try:
                line = pkt.split(b"\n", 1)[0]
                parts = line.split()
                if len(parts) >= 3:
                    expected_algo = parts[1].decode("ascii", "ignore").lower()
                    expected_hash = parts[2].decode("ascii", "ignore").lower()
            except Exception:
                expected_hash = None
                expected_algo = None
            continue
    ...
    # Verify hash if requested and provided by sender
    if raw_sha and expected_hash and hasher is not None:
        got = hasher.hexdigest().lower()
        # If algo mismatched, treat as failure
        if expected_algo and ((expected_algo == "md5" and raw_hash != "md5") or (expected_algo != "md5" and raw_hash == "md5")):
            return False
