...
def send_from_fileobj(fileobj, host, port, proto="tcp", path_text=None, **kwargs):
    ...
    if proto == "tcp":
        ...
        wait = kwargs.get("wait", False) or kwargs.get("connect_wait", False)
    ...
    if mode == "raw":
        ...
        # If requested, wait for receiver readiness (best-effort handshake).
        # This makes udp raw behave more like tcp/udp-seq for "wait".
        if (kwargs.get("wait") or kwargs.get("connect_wait")) and raw_meta and total_len is not None:
            ...
            while True:
                try:
                    ...
                    # resend META periodically while waiting
                    try:
                        sock.sendto(b"META " + str(total_len).encode("ascii") + b"\n", addr)
                    except Exception:
                        pass
        ...
        if kwargs.get("raw_ack"):
            # Stop-and-wait ACK mode (reliable but slower): PKT <seq> <data>, receiver replies ACK <seq>
            seq = 0
            ...
            while True:
                ...
                if not data:
                    break
                pkt = b"PKT " + str(seq).encode("ascii") + b" " + _to_bytes(data)
                tries = 0
                while True:
                    try:
                    ...
                    try:
                        ...
                        pass
                    tries += 1
                    if retries_max >= 0 and tries >= retries_max:
                        try:
                            sock.close()
                        except Exception:
                            pass
        ...
        if expected is None:
            if want_ack and pkt.startswith(b"PKT "):
                ...
                if seq == exp_seq:
                    ...
                    exp_seq += 1
                try:
                    sock.sendto(b"ACK " + str(seq).encode("ascii") + b"\n", _addr)
                except Exception:
                    pass
                continue
            fileobj.write(pkt)
...
def upload_file_to_internet_file(fileobj, url):
    ...
    if p.scheme in ("tcp", "udp"):
        ...
        ok = send_from_fileobj(
            ...
            mode=o.get("mode"), timeout=o.get("timeout"), total_timeout=o.get("total_timeout"),
            wait=o.get("wait"), connect_wait=o.get("connect_wait"), wait_timeout=o.get("wait_timeout") or o.get("total_timeout") or o.get("timeout"),
            window=o.get("window"), retries=o.get("retries"), chunk=o.get("chunk"),
